...
public class ManagerRoomCheck extends JFrame {
    ...
    private void deleteRoom() {
        ...
        if ("Occupied".equalsIgnoreCase(currentStatus)) {
            JOptionPane.showMessageDialog(
                this,
                "You can't delete this room because it is currently occupied.",
                "Delete Blocked",
        ...
        int response = JOptionPane.showConfirmDialog(
            this,
            "Are you sure you want to delete Room No. " + roomNo + "?",
            "Delete Confirmation",
        ...
        if (response == JOptionPane.YES_OPTION) {
            try (Connection conn = Databaseconnection.getConnection()) {
                ...
                // Log the room first before deletion
                String insertQuery = "INSERT INTO deleted_rooms (room_number, room_type, capacity, price, status, date_added) "
                                   + "SELECT room_number, room_type, capacity, price, status, date_added FROM rooms WHERE room_number = ?";
                try (PreparedStatement logStmt = conn.prepareStatement(insertQuery)) {
                    logStmt.setString(1, roomNo);
                ...
                try (PreparedStatement deleteStmt = conn.prepareStatement(deleteQuery)) {
                    deleteStmt.setString(1, roomNo);
                    deleteStmt.executeUpdate();
                }

                loadRoomsFromDatabase(); // Refresh table
                JOptionPane.showMessageDialog(this, "Room deleted and logged successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);

            } catch (SQLException e) {
                e.printStackTrace();
                JOptionPane.showMessageDialog(this, "Failed to delete and log the room!", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    ...
    private void editRoom() {
        ...
        // Get room number from selected row (assumed to be in column 0)
        String roomNo = (String) tableModel.getValueAt(selectedRow, 0);

        // Check if room is occupied
        String currentStatus = (String) tableModel.getValueAt(selectedRow, 4); // Assuming column 4 is "Status"
        if ("Occupied".equalsIgnoreCase(currentStatus)) {
            JOptionPane.showMessageDialog(
                this,
                "You can't edit this room because it is currently occupied.",
                "Edit Blocked",
                JOptionPane.WARNING_MESSAGE
            );
            return;
        ...
        int response = JOptionPane.showConfirmDialog(
            this,
            "Are you sure you want to edit this Room No. " + roomNo + "?",
            "Edit Confirmation",
            JOptionPane.YES_NO_OPTION,
            JOptionPane.QUESTION_MESSAGE
        );
        ...
        roomNo = roomNumberField.getText(); // Optional: if you're allowing room number edits
        String roomType = (String) roomTypeBox.getSelectedItem();
        String capacity = capacityField.getText();
        String price = ((String) priceBox.getSelectedItem()).replace("₱", ""); // Remove peso sign
        String status = (String) statusBox.getSelectedItem();
